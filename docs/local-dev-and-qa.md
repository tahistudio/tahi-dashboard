...
- Specs that mail through the real fan outs (e2e/sales-publish.spec.ts and e2e/public-viewers.spec.ts: a proposal decision, the contract signature notices, the signed PDF) skip unless `E2E_DEAD_RESEND_KEY=1` is set. Set it only when the server under test holds a dead Resend key: start the 3179 server with `set RESEND_API_KEY=re_e2e_harness_disabled&& npx next dev --port 3179` (cmd), then `E2E_DEAD_RESEND_KEY=1 npx playwright test -c playwright.local.config.ts e2e/sales-publish.spec.ts`. The key has to be dead rather than unset: with no key at all the contract fan out returns before it writes the signed PDF to R2, and public-viewers fails on that. The runner cannot see the server's key, so the flag is the operator's word; never set it against a server reading the live key from `.env.local`.
...
- After every deploy: curl https://portal.tahi.studio/ (307 to /sign-in when signed out), /sign-in (200) and /overview signed out (the branded not-found page, HTTP 404, which is what auth.protect() renders for a signed-out non-public route in this OpenNext setup; verified 2026-08-18 and again 2026-09-21). A 500, a Cloudflare error page or a plain-text 404 is the alarm. Then open one signed-in page in a browser. A change that passed type-check, lint, the full suite and the build once made every signed-in page throw on Workers. Roll back by reverting the merge commit and pushing (about ten minutes).
