...
- Full suite: `npx vitest run`; read the summary line (387 files on 2026-09-26). The three files that used to fail only under a full parallel run (middleware.test.ts, lib/__tests__/utils.test.ts formatDate, lib/__tests__/server-client-boundary.test.ts) were fixed at the root in LW.34 (shared mock state, the machine clock and time zone, a filesystem walk inside a test). A new failure that only shows under the full run is a new flake: keep the assertion text and log it. The gate grep must fail on the word "failed", not on the exit code of a pipe.
- Time zone checks: Git Bash drops a TZ value with a slash (TZ=Pacific/Auckland) before it reaches Node, so the run silently uses the machine zone; set `$env:TZ` in PowerShell instead.
