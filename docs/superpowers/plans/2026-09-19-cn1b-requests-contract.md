...
Cutover route (R1): POST /api/admin/task-suggestions/rebuild with `{ transcriptIds?: string[], all?: boolean }` (admin, Tahi org only). As first built it expired every pending and snoozed row of those transcripts and cleared the read mark for the next sweep. CN.1c (2026-09-26) replaced that, because two reads of one call are two samples and the second expired the first before anybody saw it. It now works like this:
- A re-read merges. By default nothing already filed is expired or changed; the writer (`insertSuggestions`, `isRepeatOf`) files only what the new read adds. Keys of rows an earlier rebuild expired are retired (af5c2991). `replace` (true, 1, or "1", "true", "yes" in any case, in the body or as `?replace=`, one rule for both) is the one path that still expires, meant for a change to what the suggester can propose: a transcript's open rows go to 'expired' (decided_by_id the caller, decided_via 'dashboard', snooze cleared, keys retired, in chunks under D1's 100 parameter cap) just before it is read or deferred.
- It reads now (`readNow`, default true) through `runSuggestionSweep` with `transcriptIds`, which reads the named calls oldest first whatever their age or read mark, twice each unless `secondPass: false`, inside the sweep's time budget (SWEEP_BUDGET_MS, 75 seconds, because workers/cron-trigger hangs up at 120 and a production read takes 20 to 27 seconds). The sweep asks the route to prepare each call only once the budget has admitted it (`beforeRead`: count or expire the open rows, retire keys, clear the mark), so a call the budget never reaches is left exactly as it was.
- What the read did not reach is deferred to the scheduled sweep: prepared and cleared, but only when the sweep will find it, that is linked to a call and at least DEFER_MARGIN_DAYS (3) inside the sweep's 30-day window (`sweepWillReach`). Anything else is left untouched and reported in `skipped`: 'unlinked' (no call, never read), 'not_found' (a named id that is not a transcript), 'too_old_to_defer' (name it again). `readNow: false` reads nothing and applies the same deferral rule to every transcript.
- Returns `{ mode, transcripts, kept, expired, read, deferred, skipped }` (`transcripts` read now or deferred, `read` the sweep summary or null), plus `readError` when the read threw.
MCP tool `rebuild_task_suggestions` mirrors it (`replace`, `second_pass`, `read_now`).
