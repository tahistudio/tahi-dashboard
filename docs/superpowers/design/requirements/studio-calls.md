...
- **Call details slide-over.** Not a sub-route: opening a row (click, or the DataTable's row-preview affordance) sets local component state (`previewCall`) and opens a `SlideOver`, no URL change. Closing returns to the plain list. Contains: an editable Title field, an editable "What it is for" select (the `meetingType` classifier: Discovery, Client check-in, Partnership, Mentoring, Other, Triage), and a `LinkedToPanel` showing Client, Deal, Lead and Request rows, each independently changeable or removable.
...
### What works today
- Unified read of every `discovery_calls` row within a ±60-day window (the page never changes the window), classified into Discovery, Client check-in, Partnership, Mentoring, Other or Triage, with a live Upcoming/Past tab split and a live search over title, lead, org and deal.
...
- **Manual reclassification is partly protected from the next calendar sync (2026-09-19).** `app/api/admin/integrations/google/sync-calendar/route.ts` now keeps a human-set `meetingType` (anything but `unclassified`) when the event has no lead/org/deal parent link, i.e. when the sync's own computed value comes only from the title heuristic (option B from the open question below, implemented for this one case). A parent link still always wins and overwrites the stored value, same as before. There is no `locked` column on `discovery_calls` in `db/schema.ts` today, so this is a rule, not an explicit lock a human can see or toggle.
