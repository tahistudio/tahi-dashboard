...
- **Claude Design write-back (DL.1, DL.2) is 11 of 14 files in**, verified line for line. The three files over about 100 KB (sales-artifacts.jsx, sales-artifacts.css, sales-pipeline.jsx) cannot go through write_files in one call; the first two currently sit TRUNCATED in the project, so the Proposals, Contracts and Schedules design pages are broken until Liam pastes the staged copies from .claude/qa/writeset/ in the editor. Detail in TASKS.md DL.1 and DL.2.
