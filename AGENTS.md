...
- **Slack**: the founders' channel is #founders (Liam and Staci only). The Slack app (CN.2) is built and deployed as the existing Tahi Dashboard app in agent mode, DM first; it waits on Liam applying the manifest and setting SLACK_BOT_TOKEN and SLACK_SIGNING_SECRET. CN.3 (thread lines, digest, reject reasons) is merged on branch cn3-ready and waits on migration 0111.
...
- `.codex/config.toml` as first created points the tahi-dashboard MCP at the dormant local `mcp-server/index.ts` and at the retired Webflow Cloud host; do not use that pair. The live MCP is the Cloudflare worker under `workers/mcp-server` (tahi-mcp-server at https://tahi-mcp-server.business-ccd.workers.dev, an OAuth front door, the same connector claude.ai uses; answered 200 on 2026-09-21) or, with a bearer token, the dashboard's own `POST https://portal.tahi.studio/api/mcp` with `Authorization: Bearer TAHI_API_TOKEN`. Liam sets tokens and secrets; never paste one into a tracked file.
...
- **Design first, then port.** New or reshaped UI is drafted in Claude Design (project "Tahi dashboard") and reviewed there by Liam before it is ported to TSX; the design sets look, density and vocabulary, the repo and the data model set the fields (never drop a real field because the mock omitted it). Tool-specific: Claude Code reaches that project through the claude-design MCP; other tools port from the requirement docs under docs/superpowers/design and the screenshots Liam shares. Exception (Liam, 2026-09-26): the 19 modules of the 2026-09-14 design pass are ported WITHOUT his review, following the app's existing patterns; every one is "ported, unchecked" until he looks, the review doc (docs/superpowers/plans/2026-09-14-design-review-for-liam.md) keeps its boxes empty and records each port commit, and the per-module plans live in docs/superpowers/design/port-plans.
...
- **Writes to production data go through the app's own endpoints as Liam**, in his browser session, dry run first where the route offers it; deletes are Liam's clicks. Never UPDATE or DELETE production D1 directly for data surgery (SELECTs and migrations through wrangler are fine where the tool permits; Claude Code's auto mode blocks migration applies on production, so there Liam runs them and the agent hands him the exact command).
...
- **Post-deploy health probe is mandatory:** curl / (307 to /sign-in when signed out), /sign-in (200) and /overview signed out (the branded not-found page with HTTP 404 is the designed answer, since auth.protect() renders not-found rather than redirecting in this OpenNext setup); a 500, a Cloudflare error page or a plain-text 404 is the alarm. Then open one signed-in page before saying "live". Roll back by reverting the merge and pushing.
- **Gate chains fail closed:** `set -o pipefail`, one && chain, `npm run type-check`, `npm run lint`, the touched vitest files (the full suite before a merge, read the summary line), `npm run build` under a timeout when a route or page changed, then push. The suite held green under full parallel runs after LW.34 (2026-09-26); a file that fails only under the full run is a new flake, so keep its assertion text and log it rather than re-running it away. Never `;` between gate stages.
