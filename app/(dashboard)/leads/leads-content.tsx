...
export function LeadsContent() {
  ...
  const [selectedCalls, setSelectedCalls] = useState<DiscoveryCall[]>([])
  ...
  async function createCall(leadId: string, body: {
    title: string
    scheduledAt: string
    durationMinutes?: number
    googleMeetUrl?: string | null
    ...
    try {
      await fetch(apiPath(`/api/admin/leads/${leadId}/calls`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    <section style={{
      ...
      {calls.length === 0 && !showForm && (
        <p style={{
          ...
          No calls yet. Schedule a discovery call to start tracking outcomes, transcripts, and scope.
        </p>
      )}
      ...
      {past.length > 0 && (
        <div>
          {upcoming.length > 0 && <SubSectionLabel>Past</SubSectionLabel>}
          {upcoming.length === 0 && past.length > 0 && <SubSectionLabel>Past calls</SubSectionLabel>}
          <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.4375rem' }}>
            {past.map(c => (
              <CallRow
                key={c.id}
...
function CallScheduleForm({
  ...
  // Default to tomorrow 10:00 in local time, formatted for datetime-local input.
  ...
  return (
    <div style={{
      ...
      <Field label="Meet URL (paste from Google Calendar)">
        <Input
          value={meetUrl}
  ...
  return (
    <li style={{
      ...
      >
        ...
        {isUpcoming && (
          <Badge tone="info" variant="soft" size="sm" dot={false}>
            Upcoming
          </Badge>
        )}
        <ChevronDown
...
function CallPostFields({
  ...
  async function save() {
    ...
    try {
      ...
      // If marking outcome for the first time, flip status to completed too.
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      ...
      <Field label="Summary (2-3 lines, the headline)">
        <textarea
          value={summary}
      ...
      <Field label="Outcome notes (what's the next step?)">
        <textarea
          value={outcomeNotes}
      ...
      <Field label="Scope notes (pages, design, integrations, etc)">
        <textarea
          value={scopeNotes}
      ...
      <Field label="Transcript (paste from Google Meet Gemini)">
        <textarea
          value={transcript}
