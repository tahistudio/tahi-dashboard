/**
 * POST /api/admin/invoices/dedupe-stripe-charges: the gates, the default and
 * the audit row.
 *
 ...
 * defaults to a dry run. The pairing and refusal logic itself is pinned in
 * lib/__tests__/stripe-charge-twins.test.ts.
 */
...
vi.mock('@/lib/stripe-dedupe', () => ({
  planStripeChargeTwinDedupe: vi.fn().mockResolvedValue({
    dryRun: true,
    pairs: [],
    refused: [],
    removedInvoiceIds: [],
...
import { POST } from '@/app/api/admin/invoices/dedupe-stripe-charges/route'
...
type Plan = Awaited<ReturnType<typeof planStripeChargeTwinDedupe>>

function makeRequest(body: Record<string, unknown>): NextRequest {
  return new NextRequest('http://localhost:3000/api/admin/invoices/dedupe-stripe-charges', {
    method: 'POST',
...
describe('POST /api/admin/invoices/dedupe-stripe-charges', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    ...
    vi.mocked(planStripeChargeTwinDedupe).mockResolvedValue({
      dryRun: false,
      pairs: [],
      refused: [],
      removedInvoiceIds: [],
  ...
  it('refuses a caller outside the Tahi org', async () => {
    ...
    expect(planStripeChargeTwinDedupe).not.toHaveBeenCalled()
  })
  ...
  it('defaults to a dry run', async () => {
    ...
    expect(vi.mocked(planStripeChargeTwinDedupe).mock.calls[0][1]).toEqual({ dryRun: true })
  })
  ...
  it('writes one audit row carrying every id removed', async () => {
    vi.mocked(planStripeChargeTwinDedupe).mockResolvedValue({
      dryRun: false,
      pairs: [{
        orgId: 'org_1',
        orgName: 'Physitrack',
        charge: { invoiceId: 'inv_2', stripeId: 'ch_1', amount: 2500, currency: 'USD', createdAt: null, paidAt: null, notes: null },
        invoice: { invoiceId: 'inv_1', stripeId: 'in_1', amount: 2500, currency: 'USD', createdAt: null, paidAt: null, notes: null },
      }],
      refused: [],
      removedInvoiceIds: ['inv_2'],
      applied: { invoicesDeleted: 1, itemsDeleted: 1 },
    } as unknown as Plan)

    await POST(makeRequest({ dryRun: false }))
    expect(logAudit).toHaveBeenCalledTimes(1)
    const entry = vi.mocked(logAudit).mock.calls[0][1]
    expect(entry.action).toBe('stripe_charge_twin_dedupe')
    expect(entry.metadata?.removedInvoiceIds).toEqual(['inv_2'])
  })
