/**
 * POST and DELETE /api/admin/proposals/[id]/share.
 *
 * Mirrors app/api/__tests__/schedule-share-snapshot.test.ts. Sharing used to
 * mint a token and set status 'shared' without ever writing a
 * publishedSnapshot. The public proposal viewer falls back to the LIVE rows
 * when there is no snapshot, so a proposal that was shared and never
 * published served whatever the studio happened to be typing: a renamed
 * package, a half-written scope, a price mid-edit. Sharing now publishes a
 * first snapshot via buildProposalSnapshot, and never overwrites one that
 * already exists, because that would silently publish edits nobody pressed
 * Republish on.
 *
 * Two consequences of that, both covered here. The response carries the
 * publishedAt it wrote, not only a boolean, because the caller keeps a local
 * copy of the proposal and its header button reads "Publish" or "Republish"
 * off that field. And revoking the link clears the published state with it:
 * POST only snapshots when there is none, so a revoke, a rewrite and a
 * re-share used to serve the client the version from the first share.
 */
import { describe, it, expect, vi, beforeEach } from 'vitest'

type Row = Record<string, unknown>

const queue: Row[][] = []
const updates: Record<string, unknown>[] = []

vi.mock('@/lib/server-auth', () => ({
  getRequestAuth: async () => ({ orgId: 'tahi-org', userId: 'user_1' }),
  isTahiAdmin: () => true,
}))

vi.mock('@/app/api/admin/_sales-access/artifact-scope', () => ({
  requireProposalAccess: async () => null,
}))

vi.mock('@/db/d1', () => ({
  schema: new Proxy({}, {
    get: (_t, table: string) => new Proxy({}, { get: (_x, col: string) => `${table}.${col}` }),
  }),
}))

vi.mock('drizzle-orm', () => ({ eq: () => ({}), asc: () => ({}) }))

function chain(): Record<string, unknown> {
  const b: Record<string, unknown> = {}
  for (const key of ['from', 'where', 'orderBy', 'limit']) b[key] = () => b
  b.then = (resolve: (rows: Row[]) => void) => resolve(queue.shift() ?? [])
  return b
}

vi.mock('@/lib/db', () => ({
  db: async () => ({
    select: () => chain(),
    update: () => ({
      set: (values: Record<string, unknown>) => ({
        where: async () => { updates.push(values) },
      }),
    }),
  }),
}))

const { POST, DELETE } = await import('../admin/proposals/[id]/share/route')

const params = { params: Promise.resolve({ id: 'p1' }) }
const post = (url = 'http://localhost/api/admin/proposals/p1/share') =>
  new Request(url, { method: 'POST' })

/** The reads the route makes: the proposal row, then (only when it has no
 *  snapshot) the cover, the sections and the variants. */
function script(existing: Row, cover?: Row, sections: Row[] = [], variants: Row[] = []) {
  queue.length = 0
  queue.push([existing])
  if (cover) {
    queue.push([cover])
    queue.push(sections)
    queue.push(variants)
  }
}

describe('POST /api/admin/proposals/[id]/share', () => {
  beforeEach(() => { queue.length = 0; updates.length = 0 })

  it('publishes a first snapshot so a shared proposal cannot serve a draft', async () => {
    script(
      { token: null, publishedSnapshot: null },
      { title: 'Website build', subtitle: 'Discovery to launch' },
      [{ id: 'sec1', title: 'Overview', position: 0 }],
      [{ id: 'var1', name: 'Package A', position: 0 }],
    )
    const res = await POST(post() as never, params)
    expect(res.status).toBe(200)
    const body = await res.json() as { status: string; published: boolean; publishedAt: string | null }
    expect(body).toMatchObject({ status: 'shared', published: true })
    // The timestamp, not only the flag: the caller flips its header button to
    // "Republish" off this field, and it had no way to learn it without a
    // full page reload.
    expect(body.publishedAt).toBe(updates[0].publishedAt)

    expect(updates).toHaveLength(1)
    expect(updates[0].publicShareToken).toBeTypeOf('string')
    expect(updates[0].publishedAt).toBeTypeOf('string')
    const snapshot = JSON.parse(updates[0].publishedSnapshot as string) as {
      proposal: Row; sections: Row[]; variants: Row[]
    }
    expect(snapshot.proposal.title).toBe('Website build')
    expect(snapshot.sections).toHaveLength(1)
    expect(snapshot.variants).toHaveLength(1)
  })

  it('never clobbers a published state on a re-share', async () => {
    script({ token: 'tok', publishedSnapshot: '{"proposal":{"title":"As published"}}' })
    const res = await POST(post() as never, params)
    await expect(res.json()).resolves.toMatchObject({
      token: 'tok', published: false, publishedAt: null,
    })
    expect(updates[0]).not.toHaveProperty('publishedSnapshot')
    expect(updates[0].status).toBe('shared')
  })

  it('clears the published state when the link is revoked', async () => {
    // Otherwise a revoke, a heavy edit and a re-share serve the snapshot from
    // the FIRST share: POST only takes one when there is none.
    const res = await DELETE(post() as never, params)
    expect(res.status).toBe(200)
    expect(updates).toHaveLength(1)
    expect(updates[0]).toMatchObject({
      publicShareToken: null,
      publicSharedAt: null,
      publishedSnapshot: null,
      publishedAt: null,
      status: 'draft',
    })
  })

  it('keeps the published state when the token is rotated', async () => {
    script({ token: 'old', publishedSnapshot: '{"proposal":{"title":"As published"}}' })
    await POST(post('http://localhost/api/admin/proposals/p1/share?rotate=1') as never, params)
    expect(updates[0].publicShareToken).not.toBe('old')
    expect(updates[0]).not.toHaveProperty('publishedSnapshot')
  })

  it('404s an unknown proposal without writing anything', async () => {
    queue.length = 0
    queue.push([])
    const res = await POST(post() as never, params)
    expect(res.status).toBe(404)
    expect(updates).toHaveLength(0)
  })
})
