...
const COVERAGE: Record<string, { source: string; status: 'filled' | 'fallback' | 'empty' }> = {
  ...
  'main-image': { source: 'cover (Flux stub)', status: 'fallback' },
  'thumbnail-image-2': { source: 'cover (Flux stub)', status: 'fallback' },
  'main-category': { source: 'strategist cluster (NOT mapped to Webflow category)', status: 'empty' },
  'other-categories': { source: '(none)', status: 'empty' },
  'schema': { source: 'JSON-LD NOT generated in round-table pipeline', status: 'empty' },
  'hreflang-block': { source: 'NOT generated in round-table pipeline', status: 'empty' },
