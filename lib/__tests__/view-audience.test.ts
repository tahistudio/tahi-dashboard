...
describe('getViewAudience', () => {
  ...
  it('reads the Client view cookie and URL-decodes the org id', async () => {
    withCookie(encodeURIComponent('4f0d2c1a-8e77-4b31-9d2a-7c5b1e6a0f33'))
    const a = await getViewAudience()
    expect(a.isPreviewingClient).toBe(true)
    expect(a.previewOrgId).toBe('4f0d2c1a-8e77-4b31-9d2a-7c5b1e6a0f33')
    expect(a.isClientAudience).toBe(true)
  })

  it('is no preview at all when the cookie could not name an org', async () => {
    // A hand-edited or half-written cookie used to count as a preview, which
    // took the studio surface away and left the operator looking for an Exit
    // button that describes a client nobody is previewing. Same rule the
    // middleware applies (lib/preview-cookie.ts).
    for (const junk of ['org acme/1', 'null', '{"orgId":"x"}', '../../etc', 'a'.repeat(65)]) {
      withCookie(encodeURIComponent(junk))
      const a = await getViewAudience()
      expect(a.isPreviewingClient).toBe(false)
      expect(a.previewOrgId).toBeNull()
      expect(a.isClientAudience).toBe(false)
