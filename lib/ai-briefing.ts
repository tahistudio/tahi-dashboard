/**
 * ai-briefing.ts \u2014 shared daily briefing generator.
 *
 * Factored out of the route handler so both the manual POST endpoint
 * (`/api/admin/ai/briefing`) and the scheduled cron endpoint
 * (`/api/admin/ai/briefing/cron`) can call it directly. The cron
 * previously self-fetched its sibling route over HTTP, which
 * Cloudflare blocked with error 1014 (workers can't loop back through
 * their own public hostname). Decision #043 updated.
 */
...
/**
 * Gather dashboard data, call Claude Sonnet, parse the response, cache
 * the result, and return the briefing.
 */
export async function generateBriefing(): Promise<BriefingResponse> {
  ...
  const overdueInvoices = await database.select({
    ...
    .where(and(eq(schema.invoices.status, 'overdue')))
    .limit(10)
  ...
  const briefingText = await callClaudeSonnet(contextText)
...
async function callClaudeSonnet(contextText: string): Promise<string> {
