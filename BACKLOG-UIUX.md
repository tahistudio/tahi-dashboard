...
- **Super-admin / Demo (Client view) mode** New entry in `SidebarUserCard` popover, visible only to super admins (`business@tahi.studio`, `staci@tahi.studio`). Switches the active session into a dummy client view used during onboarding sales calls. Stripped down to the surfaces a real client sees, with the onboarding walkthrough (loom video, checklist, where-to-go markers) wired in by default every time. Distinct from the existing client impersonation flow: this uses fixture data, not a real client account, so prices and team chatter cannot leak. Add a visible "Demo mode" pill in the top nav while active so we never confuse it with a real client session. Added 2026-05-23.

- **Super-admin / Private (Anonymised) mode** Second new entry in `SidebarUserCard` popover, same super-admin gating. Renders the full admin view but anonymises client names, contact info, prices, MRR, deal values, invoice amounts, and revenue figures. Lets us screen-share the dashboard with other agencies and friends without leaking client data. Implementation hint: wrap every "sensitive" render path in an `<Anonymise>` helper that reads a context flag, so we can layer it on incrementally rather than in one monolithic refactor. Add a visible "Private mode" pill in the top nav while active. Added 2026-05-23.
