...
- [ ] LIAM or the next session with a browser - the live pass of ed2d4385 at
  375px and in dark (2026-09-26; the Chrome extension was not connected after
  the deploy). Five of the eight LW.40 fixes passed live that morning; the
  three that failed (brief Refresh and Nudge at 44px, the Wire hanging indent,
  the contracts mobile cards) are fixed and deployed but unseen. Also unseen:
  the contract truth fixes (Lingorama SoW reads "marked signed" with no
  invented date, the admin preview pill and audit trail at 375, the dark
  status pill, Giant Group still fully signed with its real date) and the
  August fill probe (POST /api/admin/cron/snapshot-metrics?fill=2026-08,
  expected 422 nothing_derivable; May, June and July rows must be unchanged).

## Follow-ups from the 2026-09-26 batch

Reviewers' notes that were not blocking. Each names its file.

- [ ] FU.1 - [BE] Suggester: give each Anthropic read a timeout capped at the
  time left in the sweep budget, with fewer retries (lib/task-suggester.ts).
  The budget only decides when a read starts, so one hanging transcript can
  still pass the cron-trigger's 120 s cut and block the queue behind it.
- [ ] FU.2 - [BE/FE] Contracts past their expiry: a Lapsed pill on the admin
  detail and list, and the same guard on POST /api/admin/contracts/[id]/email
  and the MCP send path, so the studio cannot email a link that opens on "This
  contract has expired". The client signing email (emails/contract-sign.tsx
  about line 94) still leaves the signing deadline out of its expiry line.
- [ ] FU.3 - [Decision] A partly signed contract marked signed by hand gets no
  fully-signed email and no signed PDF although real signatures exist. Block
  the hand mark when signatures exist, or turn it into a real countersign.
- [ ] FU.4 - [FE] Expiry dates are stored as T23:59:59Z and shown in the
  viewer's zone, so a New Zealand client sees the day after the studio's pick.
- [ ] FU.5 - [FE] Dark leftovers: schedules/[id]/schedule-detail.tsx has its
  own light-hex STATUS_PALETTE; the contract detail audit timeline accents
  (contract-detail.tsx about line 1163) are light-only hex; three hand-rolled
  menus colour danger with var(--color-danger) (proposal detail, kanban board,
  messages); the Wire timestamp and News feed label use --text-faint (3.2:1).
  The new --color-danger-ink token is the fix for the danger cases.
- [ ] FU.6 - [FE] Desktop: a long Wire item wraps inside the fixed 1.6rem strip
  and its second line is clipped (wants a one-line ellipsis); the contracts
  table at 1280 is wider than its card, so Sent, Expiry and Row actions sit
  behind a sideways scroll.
- [ ] FU.7 - [BE] app/api/admin/overview/brief/route.ts exports non-route
  symbols (computeBrief, readBriefCache, writeBriefCache, isBriefFresh,
  BRIEF_CACHE_KEY); type-check fails whenever a dev server's .next/types is
  present. Move them into lib/.
- [ ] FU.8 - [Slack] A voice note in the DM arrives twice (message.im with the
  file inline and file_shared), so one recording may give two cards; check in
  the first live DM. Before channels go live, send the channel pointer and the
  stranger refusal with chat.postEphemeral.
- [ ] FU.9 - [QA] The seeded QA sqlite lost its FK cascades, so the D3 specs
  leave contract_signers, contract_signatures and proposal children behind
  (cleanup command in docs/local-dev-and-qa.md); reseed with cascades intact.
...
- [x] LW.34 - [QA] Flaky under a full parallel run, green in isolation: middleware.test.ts (all 8, shared mock state) and lib/__tests__/utils.test.ts formatDate (clock or locale). Isolate the mocks and freeze the clock so the gate never lies. 2026-09-19: lib/__tests__/server-client-boundary.test.ts also times out under the full parallel run (6.8s against a 5s budget) and passes alone; the lead gate now retries a failed file in isolation before calling the suite red. Fixed 2026-09-26 (0e938405, test files only, no product bug). Causes: (1) heavy I/O inside a test's 5s budget: the boundary scan re-read an import target on each of ~3,300 imports (14.4s with three suites at once), and no-resend-bypass (4.9s), invoice-drafts-not-owed (2.7s) and studio-page-audience (1.7s) walked and read the tree inside their tests; now one Dirent walk and one read per file at collection, each rule re-proved with a planted offender. (2) module loads inside tests: middleware.test.ts imported @/middleware (real Clerk via importOriginal) in every test, so one slow load timed out every test awaiting it (reproduced), and dashboard-guide.test.ts re-imported next/server after vi.resetModules six times; now static imports and vi.hoisted mocks, with vi.resetAllMocks so a queued mockRejectedValueOnce cannot leak. (3) the clock: formatDate's '3d ago' compared two real-clock reads on an exact day boundary (a backwards clock step gives '2d ago'), and admin-invoice-numbers expected the runner's local year where the route mints the Auckland year (wrong on a UTC runner 13 hours every New Year); Date is frozen in both, green from UTC-12 to UTC+14. The 2026-09-14 failure output was not kept, so (2) and (3) are the mechanisms that fit it, not a replay. Proof: 377 files / 5,715 tests green on 4 consecutive full runs, and 3 of 3 green with three suites running at once (0 of 3 before); slowest test under that load 705ms.
...
- [~] DPM.1 (renamed from PM.1 on 2026-09-26 so PM.1 can mean the product manager AI's check; the run log and STATUS still call it PM.1) (MERGED e9feda94, live, both defaults set to Liam on production 2026-09-14 00:05 NZ; Liam to confirm from Settings) - [BE/FE] **Default project manager for new clients, by engagement
