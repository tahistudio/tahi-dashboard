...
- [~] CN.0 - [BE/FE] Foundations (about 3 days): call_transcripts for every call kind (the Drive sync writes there and matches scheduled calls too; unmatched notes parked as unlinked for Liam to attach from the calls page); the Tahi bot actor (authorType bot); task_comments and a Task thread on the task detail, mirrored into the request thread when the task has one. In flight.
