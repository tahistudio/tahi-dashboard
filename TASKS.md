...
## Clients redesign (Liam, 2026-09-05: "hecticly bad design right now")

- [x] CR.1 - [Design] Clients list and client detail designed in Claude Design (project Tahi dashboard: clients.jsx, clients-kit.jsx, clients-detail.jsx, clients-data.jsx, clients.css, wired into the shell). Critic: keep Deals/Time/Revenue/Profitability via a Money tab, keep the status filter and an Archived view. 2026-09-06.
- [x] CR.2 - [FE] Clients LIST ported (0b9cd35a, merged 382993d0): RailLayout saved views (All, Active retainers, Projects, At risk, Paused, Archived) with true counts, filters (status, plan, health, owner, tag, tracks) that go to the endpoint, sort, list and cards views, health reasons in a keyboard and touch reachable tooltip, MRR for admins from retainer-health with an honest fallback, whole checkbox cell selects, bulk bar (assign owner, add tag, export CSV, archive), server paging with a page bar, per-row Open / View as client / Invite / Archive or Restore, mobile card layout with a 2.75rem actions menu, New client panel with labelled fields. Live smoke on production 2026-09-06 04:00 NZST: rail, counts (28 clients, Archived 26), cards on a phone, no overflow, no errors.
- [x] CR.3 - [FE] Client DETAIL ported (05b44b0c, merged 86532f37): client-detail.tsx split into tabs/* and _kit/* with dead code removed, then the redesign: hero (plan and tracks, billing, health with reasons, owner menu, next call), Needs you strip (money-gated), Work in flight tracks, nine tabs (Overview, Requests via the shipped kit with sub-requests filtered and honest custom-column badge, Invoices with per-currency sums and one overdue predicate, Files with working serve links, People, Papers, Calls with honest attendee copy, Money = Revenue + Profitability + Time + Deals, Settings incl. IC.2 channel and terms, brands CRUD behind a Confirm, onboarding rail card), dark-mode badge tokens, tablist keyboard focus, ?tab= sync. Render-checked on the QA server (all nine tabs, 375px, dark) before push.
- [x] CR.4 - [FE] Mobile touch polish (17a490b2): every TahiButton size clears 2.75rem below md via .tahi-btn-sm/-md/-lg in @layer components (design sizes from 48rem up), breadcrumb parent link and EmptyState CTA the same plus focus rings, the Clients mobile card title button 2.75rem, the hero action row never orphans its overflow, the More sheet Track time row left-aligned with the other rows. Re-probed on the QA server at 375px: zero under-44 controls on the client detail, only the search box's inner input on the list. Deployed.
