...
- [~] LW.31 - [BE/FE] Prep notes on calls: the brief says "no prep note yet" with nowhere to write one. Agent call-prep-notes: prep note on both call kinds, editable from the brief link and the calls slide-over, in the pre-call digest, MCP fields.
- [~] LW.32 - [BE/FE] A member seat must not see the plan or billing on the client home (Liam: "I shouldn't see my plan on the home screen if I'm a member"). Agent member-plan-hidden: server-side omission for member seats, admin seats unchanged.
