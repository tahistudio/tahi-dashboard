...
- [~] CN.2 - [BE/Slack] The Slack app, DM first with permission levels (Liam, 2026-09-22: the bot lives under Apps, everyone talks to it 1:1, founders, team members and clients at different levels; channels later). Contract docs/superpowers/plans/2026-09-22-cn2-slack-app-contract.md, manifest docs/superpowers/plans/2026-09-22-slack-app-manifest.md. In flight as workflow cn2-slack-app: S1 identity, verification, routes; S2 approvals in DMs with in-place rewrites; S3 notes and voice in; A1 assignee suggestions on every kind. Liam applies the manifest and sets SLACK_SIGNING_SECRET.
...
- [~] LW.43 - Crons fire from a Cloudflare Worker, not GitHub Actions (2026-09-22). GitHub dropped most scheduled runs (measured 2 to 5 an hour against 12 or more configured), so the suggester never ran after 19 Sep and Liam's Charles call produced no suggestions until a manual sweep (9 filed). workers/cron-trigger with Cron Triggers mirroring the yml schedule table; the yml keeps only the manual dispatch. Needs the two worker secrets (DASHBOARD_URL, TAHI_CRON_SECRET) set once.
