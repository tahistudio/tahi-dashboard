...
- [~] CN.1d - [BE/FE/MCP] Suggestions never duplicate what exists (Liam, 2026-09-19: make sure it checks for requests that are close or similar). Similarity scoring in lib/text-similarity.ts; the inbox warns Looks like #226 with Use #226 instead and Approve anyway; approve is blocked at 0.8 without force; the sweep drops cross-call duplicates; the model also sees requests delivered in the last 90 days. Contract docs/superpowers/plans/2026-09-19-cn1d-duplicate-guard-contract.md; in flight as workflow cn1d-duplicate-guard.
