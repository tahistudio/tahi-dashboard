 ...
 *
 *   C2 (public viewer integrity). At 375px every package tab is reachable (the
 *   third one used to clip off the strip with no way to scroll to it), each is
 *   a 44px touch target, and nothing scrolls the page sideways.
 *
 ...
 * would leave an archived row in the real pipeline, because deal DELETE is a
 * soft archive.
 *
 ...
 * that one is refused by Resend. Never point this file at a server holding a
 * live key. The acceptor address is on example.com, which the allowlist never
 * passes.
 *
 * Data: every proposal is created here under a unique title and deleted in a
 * finally, and its sections, variants and acceptances go with it through the
 * ON DELETE CASCADE the migrations declare. The seeded QA sqlite lost nearly
 * every FK clause in the export it came from (three of its 122 tables keep
 * one), so on that harness the child rows outlive the parent, unreachable from
 * any list. The bell rows are marked read, since the bell has no delete.
 *
...
/** True when the element sits wholly inside the viewport's width. */
async function isHorizontallyInView(locator: Locator): Promise<boolean> {
  ...
  test('share pins a snapshot, publish re-arms it, and an accept at 375px reaches the studio', async ({ page, request, studio }) => {
    ...
    try {
      ...
      for (const name of ['Starter', 'Growth', 'Scale']) {
        ...
        await expect.poll(() => isHorizontallyInView(tab), { message: `the ${name} tab is clipped` }).toBe(true)
