...
- **Deviations / deferred.** No custom OAuth interstitial or success card (Clerk handles the redirect). First/last name fields appear only if the Clerk instance is set to collect them (dashboard setting, not code). Trust avatars are placeholder swatches - swap in real client logos/photos later. Live smoke on the deployed URL (mobile 375px + dark mode) still pending per Definition of Done.
