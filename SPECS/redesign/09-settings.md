...
6. **Audience-shaped, server-gated.** A section a person cannot use is absent, never disabled (05 deny-by-default); the server is the gate and the UI never renders a locked door. This is 05's bidirectional invariant applied to Settings itself: what you can see here you can open, and what you cannot open never appears.
...
| `/settings/integrations/announcements` | Announcements | owner | new composer + management (backend routes exist; email send path incomplete, see the Announcements spec) |
...
**Announcements (`/settings/integrations/announcements`)**
- *Purpose:* Tahi broadcasts a change to clients. One place to compose a message, choose who hears it, send it as an in-app banner (and optionally an email), and watch how far it reached. This is the outbound-comms twin of Automations ("when this happens, do that"); it sits beside it in the group. The `announcements` + `announcementDismissals` tables and the portal banner / dismiss endpoints already exist (see the backend note below); this section is the missing admin face.
- *Layout:* the wide 58rem pane, split into two stacked zones: the **composer** (a batched section behind the save bar, since it is free text plus coupled fields) at the top, then a hairline, then the **past announcements** list below.
- *Composer anatomy (top to bottom, each a section-frame form row, 40rem control column):*
  - **Audience** - a segmented control (the standard three-way): "All clients" | "By plan" | "Specific clients". Selecting "By plan" reveals a plan select (the plan-type values, e.g. Maintain / Scale) below it; selecting "Specific clients" reveals a multi-select client picker with search and removable chips (each chip = an org name; the leaf budget does not extend to chips, they use `--radius-md`). "All clients" reveals nothing further. A live reach caption sits under the control: "This reaches N contacts across M clients." (0.75rem muted, recomputed as the audience changes). Maps to `targetType` (`all` / `plan_type` / `org`), `targetValue` (the plan), and `targetIds` (the org id array).
  - **Type** - a quiet segmented control: Info | Success | Warning | Maintenance. Sets the banner and email accent (`type`); does not change the forest strip's base, only its leading icon and the email eyebrow.
  - **Title** - a single-line input (the banner's bold lead and the email subject). `title`.
  - **Message** - a textarea (3 rows, grows), plain sentences; the helper reads "Blank lines start a new paragraph in the email." `body`. No raw HTML: the field is plain text and renders escaped in both surfaces.
  - **Link (optional)** - URL input + a short "Link label" input beside it (e.g. "See what changed"); becomes the banner's inline link and the email's leaf button. Empty = no link.
  - **Also send email** - the standard toggle (label "Also send email", helper "Targeted contacts get a branded email as well as the in-app banner."). Off = banner only. Maps to `sentByEmail`.
  - **When** - a segmented control "Send now" | "Schedule"; "Schedule" reveals a datetime field (maps to the dormant `scheduledAt`; flag in copy that scheduling is a planned follow-up if the send path ships now). Below sits an optional **Dismissible until** datetime (helper "After this the banner auto-hides for everyone." maps to `expiresAt`; empty = shows until each client dismisses it).
- *Live preview (right of the composer at >= 54rem pane, stacked below it under that):* a two-tab preview card ("Banner" / "Email"), white surface, hairline, `--radius-lg`, 1.25rem padding.
  - **Banner tab:** a faithful render of the client strip below (the forest-gradient `.imp-banner` look) using the current title, message, link, and type icon, with the X present but inert. Caption: "This is what clients see at the top of the portal."
  - **Email tab:** a scaled card echoing the React Email template (forest header band, "Announcement" eyebrow tinted by type, title, message paragraphs, leaf CTA if a link is set, footer). Caption: "Only sent when 'Also send email' is on." When the toggle is off the Email tab dims and reads "Email is off for this announcement."
- *Composer states:* clean / dirty (save bar: primary reads "Send now" or "Schedule" per the When control, not "Save changes", since committing publishes) / sending ("Sending...") / sent (save bar drops + toast "Announcement sent to N contacts" or "Banner published to N contacts" when email is off) / validation (empty title or message blocks send, inline helper) / error (bar stays, "Could not send. Nothing was published.").
- *Past announcements list anatomy:* hairline rows, 4rem (two lines): line one = title (0.875rem 500 ink) + a quiet type chip + an audience chip ("All clients" / "Maintain plan" / "3 clients"); line two = sent-relative time (0.75rem muted) and, right-aligned and tabular, the **reach + dismiss stats**: "Reached 24" then "9 dismissed" (0.75rem muted; dismissed count = `announcementDismissals` rows for that id), and an "Emailed" quiet chip when `emailSentAt` is set. Right cluster: a live/expired status dot (published + within `expiresAt` = success dot "Live"; past `expiresAt` = muted "Ended"; unpublished draft = warning "Draft"), and quiet icon buttons. A small stacked-bar or ring next to the numbers may show dismissed / reached as a proportion (one accent, no second hue).
- *Past list states:* loading (skeleton rows) / populated / empty ("No announcements yet. Your first broadcast will appear here." + the composer above is the CTA) / a row mid-delete (confirm first: "Delete this announcement?" / "It disappears from every client's portal. Dismiss history is removed." per the cascade).
- *Note on editing:* the backend has no PATCH/DELETE for announcements yet (see below); until they exist, past rows are read-only with a "Duplicate into composer" affordance rather than an inline Edit, so the flow never promises a save it cannot make.

**Client announcement banner (portal surface, not a settings route)**
- *Purpose:* the client-facing half of an announcement. A dismissible forest strip at the top of every portal page, reusing the exact "view as client" impersonation-banner look so it reads as part of the same forest system. Specced here because Settings composes it and its preview must match it pixel for pixel; it mounts in the dashboard layout beside `ImpersonationBanner` (the client-only slot), not on a settings route.
- *Anatomy:* the `.imp-banner` forest-gradient strip (linear-gradient `#1e2a1b` -> `#243121`, 1px `#2d3d2a` bottom border, the sub-pixel green glow underline, `#eaf3e4` ink, 500/13px Manrope, `flex-shrink:0` so it never compresses the scroll area). Left: a leading type icon (`.imp-eye` `#9ec495`, glyph per type - info / check / alert-triangle / wrench), 0.6875rem gap, then the title in `.imp-banner b` weight-700 white followed by the message in the base ink, then the optional inline link (underlined `#cfe6c7`, hover white). Right: the dismiss control reusing `.imp-exit` (a bordered pill, "Dismiss" label or an X icon, `margin-left:auto`). These are the sidebar-exempt hardcoded hex, identical in light and dark, so no dark-mode overrides.
- *Stacking:* if a client is targeted by more than one live announcement, banners stack vertically (newest on top), each independently dismissible; a soft cap of the newest two, with the rest reachable but not stacked, keeps the strip from swallowing the page.
- *Dismissal (the load-bearing behaviour):* pressing X dismisses the banner **forever for that person**, persisted server-side by POSTing `/api/portal/announcements/[id]/dismiss` (writes `announcementDismissals` keyed by Clerk `userId`), so it stays gone across devices; the portal GET already filters dismissed rows on reload. Optimistic removal is fine (the strip slides up and out over 200ms); localStorage may back it as an instant offline fallback but is not the source of truth. An announcement also auto-hides for everyone once past its `expiresAt`.
- *States:* hidden (none live, or all dismissed) / one banner / stacked / dismissing (slide-up 200ms, then removed) / dismiss-failed (banner returns, no error chrome; it will simply reappear on next load). `prefers-reduced-motion`: the slide becomes an instant removal.
- *Accessibility:* the strip is a `role="region" aria-label="Announcement"`; the dismiss control is a real button with `aria-label="Dismiss announcement"`; the link is a real anchor; the ink-on-forest pairing meets AA at this size (verify `#eaf3e4` on `#1e2a1b`).
...
- `Announcements` - "Broadcast a change to clients, by plan or by name."
...
**Announcements:** audience `All clients` / `By plan` / `Specific clients`; reach caption `This reaches N contacts across M clients.`; type `Info` / `Success` / `Warning` / `Maintenance`; labels `Title`, `Message`, `Link (optional)`, `Link label`; message helper `Blank lines start a new paragraph in the email.`; email toggle `Also send email` + `Targeted contacts get a branded email as well as the in-app banner.`; when `Send now` / `Schedule`; dismissible label `Dismissible until` + helper `After this the banner auto-hides for everyone.`; preview tabs `Banner` / `Email`; preview captions `This is what clients see at the top of the portal.` / `Only sent when 'Also send email' is on.`; email-off preview `Email is off for this announcement.`; primary `Send now` / `Schedule`; sending `Sending...`; sent toast `Announcement sent to N contacts` / banner-only `Banner published to N contacts`; send error `Could not send. Nothing was published.`; list empty `No announcements yet. Your first broadcast will appear here.`; row stats `Reached N` / `N dismissed`; row status `Live` / `Ended` / `Draft`; emailed chip `Emailed`; row action `Duplicate into composer`; delete confirm `Delete this announcement?` / `It disappears from every client's portal. Dismiss history is removed.` / `Cancel` / `Delete`.
**Client banner:** dismiss control `Dismiss` (aria `Dismiss announcement`); it stays gone for that person on every device once dismissed.
...
7a. **Announcements:** the composer (audience segmented to "By plan" with the plan select shown, type + title + message + optional link, "Also send email" on, "Send now") with the live Banner / Email preview, and below it the past-announcements list with reach + dismiss stats and status dots. Plus the client-side forest banner on a portal page (single and stacked), and its slide-up dismiss.
...
- Team & access is where the owner's north star lands: onboarding a person is template-first. The invite flow on Team members assigns a role at invite time and offers "Copy access from" an existing member (05's replicability primitive); a new subject never starts from a blank slate of toggles.
...
6. **Modules vs the feature tree (confirmed finding):** `module_*_enabled` settings keys and `featureVisibility` (05) are two parallel gating systems answering "is this area on". Decide: fold Modules into the permissions engine as workspace-level defaults, or keep it as a distinct kill switch and document the precedence. Do not ship the redesign with both silently coexisting. Two parallel systems answering the same question is exactly how the map starts lying (05's invariant); the reconciliation belongs to the launch-blocker enforcement work, not cosmetics.
...
11. **Announcements backend gap (blocks the composer from being fully honest).** The schema (`announcements` + `announcementDismissals`), the admin list/create routes, the `[id]/send` route (publishes and, when `sentByEmail` is on, resolves targeted contacts for all three audience modes and emails each via Resend), the portal banner GET, and the portal dismiss POST all exist. But: (a) **email is dead code** - nothing ever writes `sentByEmail`, so `/send` publishes but never emails; the create POST body and the MCP `create_announcement` tool both omit the flag. The composer's "Also send email" toggle needs the create/send path to actually persist and honour it, and the send route should render the branded `emails/announcement.tsx` template (to be created) instead of the current raw inline-HTML string (which also interpolates `body` as unsanitised HTML - an XSS risk the plain-text field + escaped render fixes). (b) **No PATCH or DELETE** route exists, so past announcements cannot be edited or deleted; hence the list is read-only with "Duplicate into composer" until those routes ship. (c) **`scheduledAt` is inert** (no reader, no cron), so the composer's "Schedule" option is a planned follow-up, not a live path; ship "Send now" first. (d) **MCP `create_announcement`** cannot set `targetIds` (org-list targeting), `expiresAt`, `publish`, or the email flag, and passes `body` where the route reads `content` - reconcile the tool with the composer's fields. None of this blocks the design; it scopes what the first shippable slice of the composer can truthfully do (banner to all three audiences now; email once the flag is wired; edit/delete/schedule as fast-follows).
