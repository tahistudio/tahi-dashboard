...
export const TOOLS: ToolDef[] = [
  ...
  tool(
    ...
    'ANSWERS 403 FORBIDDEN UNDER THIS SERVER\'S SERVICE TOKEN, ALWAYS, INCLUDING THE DRY RUN. The route requires super_admin and the MCP service identity resolves to admin, by design: an irreversible delete of a client and its invoices wants a human identity behind it. If you call this and get 403, that is the design, not a misconfiguration; do not retry and do not report a permissions bug. Tell the user to run it from the client\'s Settings tab, Danger zone, as a super admin. Documented here so the contract is discoverable: DELETE /api/admin/clients/{clientId} removes an organisation AND ALL ITS ORG-SCOPED ROWS INCLUDING ITS INVOICES, INVOICE ITEMS, SUBSCRIPTIONS AND TRACKS, DRY RUN BY DEFAULT. It exists for dummy rows carrying test invoices, which the import cleanup refuses over. confirmName must equal the organisation\'s current name EXACTLY or the route answers 400. It answers 409 and names what is wrong when the organisation carries a manyrequests_id (the import adopted it), a clerk_org_id (a real login exists), any contact with a clerk_user_id (a real person can sign in), any deal, lead, CRM activity, discovery call, contract, contract document, proposal, project schedule or project calculation (pipeline and sales artefacts are always real), or a paid invoice on a live Stripe or Xero rail while the organisation also holds a Xero contact id (a real ledger: merge it instead). A Stripe customer id alone is NOT a refusal, because a test-mode customer looks exactly like a live one, but the dry run prints it along with every invoice\'s rail id, amount and date. Both login refusals ask Clerk first: a clerk_org_id or a clerk_user_id that answers 404 in Clerk is a STALE column and refuses nothing, while a Clerk that cannot be reached keeps the refusal (fail closed). removeClerkOrganisation is the one narrow exception to the two login refusals, for an accidental workspace: see its own description. Archive is the reversible answer and is right for anything uncertain. Sends no email of any kind.',
    {
      ...
      removeClerkOrganisation: prop('boolean', 'DEFAULT FALSE. The accidental-workspace door: the organisation self-serve provisioning creates when a Tahi teammate accepts a team invite with no active organisation, which is otherwise unremovable because a Clerk org id and a signed-in contact are both refusals. True REPLACES those two refusals with a stricter eligibility rule, checked before anything is written: the caller is a super admin, the Clerk organisation is NOT NEXT_PUBLIC_TAHI_ORG_ID (the studio organisation is never deletable), EVERY contact carrying a clerk_user_id has an email matching a team_members row case-insensitively (so the only logins attached belong to Tahi staff), and the organisation holds no invoice, no Stripe customer or Xero contact id, no ManyRequests id and none of the pipeline rows the ordinary delete refuses over. Any check fails and it refuses with the reason and deletes nothing. When eligible and dryRun is false the CLERK ORGANISATION IS DELETED FIRST (a Clerk 404 counts as already gone; any other Clerk error answers 502 and writes nothing) and only then the database rows. The dry run reports eligibility, the Clerk org id that would go and the contact emails that would be unlinked, and the audit row records both.'),
    },
  ...
  tool('rebuild_task_suggestions', "Re-read call transcripts the suggester has already looked at, and add what the new read finds to what is already waiting. Nothing already filed is expired or changed: pending and snoozed suggestions stay in the inbox, and applied, rejected and failed ones are decisions and are left alone. The new read files only what is new; an item it proposes again, in the same words or different ones, is counted as a duplicate and the existing suggestion is kept, and nothing a founder already rejected comes back. The transcripts are read straight away, oldest first, for as long as fits in about 75 seconds (roughly one call, since each is read twice: the second read is shown the first and asked only for what it missed), and the sweep summary comes back as read. The rest are deferred: left for the scheduled sweep, which runs every 30 minutes and reads about one call a run. A transcript is only touched when it is read or deferred, and only deferred when the sweep will find it; one older than about 27 days that did not fit is left exactly as it was and listed in skipped as too_old_to_defer (name it again), and so are transcripts with no linked call (unlinked, never read) and ids that are not transcripts (not_found). Returns mode, transcripts (read now or deferred), kept (undecided suggestions left in place), expired, read, deferred and skipped (each with its reason). replace true is the escape hatch for a change to what the suggester knows how to propose: it expires the still-undecided suggestions of each transcript just before it is read or deferred, so the re-read starts from nothing. Costs roughly 10 cents per transcript read with both reads.", {
    ...
    replace: prop('boolean', 'Expire the still-undecided suggestions from these transcripts before re-reading, instead of adding to them. Default false. Only for a change to what the suggester can propose.'),
    second_pass: prop('boolean', 'Read each transcript a second time with the first read shown ("anything missed?"). Default true.'),
    read_now: prop('boolean', 'Read what fits straight away and return the summary. Default true; false reads nothing here and defers every transcript the scheduled sweep will find (the rest come back in skipped).'),
  }),
  ...
  // ── Write: Invoices ───────────────────────────────────────────────────
  tool('create_invoice', 'Create a new invoice for a client. The invoice NUMBER is minted here from the studio sequence (the settings key invoice_number_prefix plus a global counter) and comes back as `number` alongside the id: it is not a field you supply, and it is what gets pushed to Xero as the InvoiceNumber if the invoice is later synced. A null number means the counter could not be reached and the row is unnumbered, which is safe but worth reporting. Note that lineItems is what the route actually bills from; orgId plus a bare amount is not enough to raise an invoice.', {
    orgId: prop('string', 'Client organisation ID'),
  ...
  tool('backfill_invoice_numbers', 'Recover the invoice number an IMPORTED invoice was born with, for rows raised before invoice numbering existed. It reads the number back out of what the importer of the day recorded: the ManyRequests key (which IS the invoice number on that system), or the number the Xero and Stripe importers wrote into the notes. It NEVER mints a number from the studio sequence for a historical row, never renumbers a row that already has one, and never guesses: anything it cannot recover comes back in `refused` with a reason and a sentence. DEFAULTS TO A DRY RUN, because this writes a money column under a unique index: it returns the exact plan and touches nothing unless you pass dryRun false.', {
    dryRun: prop('boolean', 'Preview only. Defaults to TRUE: pass false to actually write the recovered numbers.'),
  }),
  tool('update_invoice', 'Update an existing invoice: status, notes, due date, owner and the paid / sent dates. Amounts and line items are not editable here, delete the invoice and create a fresh one instead. This does NOT send it: flipping an invoice to the client is send_invoice_email, which is the only door that emails the billing contacts and raises their bell row. Moving the status to paid PUSHES BACK TO THE RAIL after the local write: a Xero-sourced invoice gets a payment recorded against the bank account code in the setting invoicing.xeroPaymentAccountCode (skipped, not guessed, when that is unset), and a Stripe-sourced one is marked paid out of band in Stripe. The Xero payment is for the balance XERO says is outstanding, read back with a GET before posting, never the dashboard total: a dashboard-raised NZD invoice carries 15% GST in Xero that the local total does not, and Xero would silently take the short amount as a partial payment. A Xero invoice that is not AUTHORISED (a draft awaiting approval in Xero, which is where every pushed invoice starts) or that has nothing outstanding is a reported skip, not a payment. The push never blocks the local write and its outcome comes back as pushback { rail, status: done | skipped | failed, reason? } and is written to the audit log whenever it is not done. Pass pushback false to record the payment locally only, for money the rail already knows about. Only a real transition into paid on a row with no paid date yet pushes, so neither a repeat patch nor a paid to written_off to paid round trip can double-post.', {
    invoiceId: prop('string', 'Invoice ID'),
  ...
  tool('send_invoice_email', 'Send an invoice to the client: emails every billing contact (portal admins + the primary contact) the invoice with a pay link and a portal deep link, marks the invoice sent, and raises the client bell row. The subject, the emailed copy and the bank reference all quote `number` when the invoice has one, so the client, Xero and the dashboard all name the bill the same way. Creating a draft invoice does NOT notify anyone; this is the send. The pay link is the Stripe hosted page, or Xero\'s own online invoice once the bill has been approved in Xero; when neither exists and the client bills on the Xero rail the email carries a How to pay block instead (the studio bank account for the INVOICE\'s currency from invoicing.bankDetailsByCurrency, falling back to the single account in invoicing.bankDetails when that currency has no entry, plus the invoice number as the reference), so a client is never sent a bill with nothing to act on and a GBP bill is never quoted an NZD account. WHO SENDS on the Xero rail is the setting invoicing.xeroEmailMode: dashboard (the default) sends our template only; xero calls Xero\'s own Email endpoint and stands our template down; both sends each. Xero refuses to email an invoice that is not AUTHORISED, and every dashboard-pushed invoice starts as a Xero DRAFT, so a refusal falls back to our template rather than leaving the client with nothing. Safe to retry: calling this again re-sends OUR email (which is what a resend is for) but will not make Xero mail a second PDF, because the Xero half stands down once Xero\'s own SentToContact flag and our sentAt both say it has gone. The response reports what happened: xeroEmail sent | skipped | failed with a reason, alongside sentTo, failedTo, payLink and bankDetails (true only when the client was actually given somewhere to send the money, i.e. the resolved account names a bank, an account name, an account number or an IBAN).', {
    invoiceId: prop('string', 'Invoice ID'),
  }, ['invoiceId']),
  ...
  tool('create_deal', 'Create a new deal. Split-value model: upfrontValue (one-off project portion) + monthlyValue (recurring retainer). Either or both can be set. Range on upfront via valueMin/valueMax (uses midpoint as upfrontValue when set). recurringStartDate optional — when null, falls back to engagementEndDate, then expectedCloseDate.', {
    ...
    orgId: prop('string', 'Client organisation ID'),
    upfrontValue: prop('number', 'One-off / project portion (e.g. $30k for a 3-month build)'),
    monthlyValue: prop('number', 'Recurring monthly retainer (e.g. $2k/mo)'),
    recurringStartDate: prop('string', 'When the retainer kicks in (YYYY-MM-DD). Optional.'),
    valueMin: prop('number', 'Low end of upfront range (use with valueMax)'),
    valueMax: prop('number', 'High end of upfront range (use with valueMin)'),
    value: prop('number', 'Legacy single value field — accepted for back-compat, treated as upfrontValue if upfrontValue is unset'),
  ...
  tool('update_deal', 'Update a deal. Pass any subset: upfrontValue/monthlyValue/recurringStartDate (split-value model) or legacy value+valueMin/valueMax. valueChangeNote is logged in the timeline (e.g. "Scope grew").', {
    dealId: prop('string', 'Deal ID'),
    stageId: prop('string', 'New pipeline stage ID'),
    upfrontValue: prop('number', 'Updated one-off portion'),
    monthlyValue: prop('number', 'Updated monthly retainer'),
    recurringStartDate: prop('string', 'Updated recurring start date (YYYY-MM-DD); pass empty string to clear'),
    value: prop('number', 'Legacy: updated single-value deal value (treated as upfrontValue when split fields unset)'),
    valueMin: prop('number', 'Low end of upfront range (pass null to clear)'),
    valueMax: prop('number', 'High end of upfront range (pass null to clear)'),
    ...
    engagementType: prop('string', 'Engagement type: project or retainer (capacity planning only; values are governed by upfrontValue/monthlyValue)'),
    totalHours: prop('number', 'Total project hours (capacity)'),
    hoursPerMonth: prop('number', 'Monthly retainer hours (capacity)'),
  ...
  tool('send_nudge', 'Send a nudge email to deal contacts. The email delivery allowlist filters the list per address: the response carries suppressed (the addresses held back) and suppressedCount, and the deal timeline names only the addresses actually delivered to. A nudge with nothing left answers 409.', {
    dealId: prop('string', 'Deal ID'),
  ...
  tool('cron_suggest_from_transcripts', 'Call notes to tasks: reads the call transcripts nobody has read yet (5 by default, up to 20 with limit, oldest first, last 30 days), asks Sonnet what the call changed, and writes task_suggestions rows for approval. Never applies anything. Each call is read twice by default: the second read is shown what the first proposed and asked only for what it missed, and its new items join the first read\'s through the same duplicate checks (roughly 3 cents more per call at Sonnet 5 rates, the notes being read back from the prompt cache). The gate: the call must belong to a client (an org on the call, or meeting_type client). Every transcript it looks at is stamped suggested_at, including skipped and failed ones, so no sweep picks the same transcript up again. A run only starts a call whose reads should finish within about 75 seconds (the scheduler gives up at 120), so in practice it reads one or two calls; the ones it did not reach stay unread and unstamped for the next run and are counted in deferred (named in deferredIds). A second read that would not fit is skipped and the first read is still written. Also returns snoozed suggestions whose time has come to pending. The summary reports secondPass: ran (second reads that came back), proposed, failed, and skipped (full, or out_of_time).', {
    limit: prop('number', 'Transcripts this run may take on, 1 to 20. Default 5. The 75 second budget still applies, so a larger limit mostly means more deferred.'),
    second_pass: prop('boolean', 'Read each call a second time with the first read shown ("anything missed?"). Default true; pass false to read each call once this run.'),
  }),
  ...
  // ── Settings ──────────────────────────────────────────────────────────
  tool('get_settings', 'Get all dashboard settings as a key/value map. invoicing.defaultChannel and invoicing.xeroEmailMode are always present, filled in with their studio defaults (stripe, dashboard) when no row holds a value. The five email delivery keys are always present too, in their RESOLVED form: email.deliveryMode ("allowlist" | "all", and an absent or malformed row reads as allowlist, so the gate is on), email.allowedDomains (a JSON array, ["tahi.studio"] when unset), email.allowedAddresses (a JSON array of exact mailboxes, ["business@tahi.studio"] when unset; while it holds anything an address must be on it AND on an allowed domain), email.blockedAddresses (a JSON array checked before every other rule including mode "all", ["staci@tahi.studio","nathan@tahi.studio"] when unset) and email.allowedOrgIds (a JSON array, [] when unset; an exempt client is mailable at that client\'s own contact addresses, not at any address on a send that mentions them).'),
  tool('update_settings', 'Update a dashboard setting. Any key is accepted; thirteen are validated and answer 400 with a sentence when the value is wrong: invoicing.defaultChannel (stripe | xero), invoicing.xeroEmailMode (dashboard | xero | both, who emails a Xero-rail invoice), invoicing.xeroPaymentAccountCode (a Xero bank account code, up to 10 letters, digits or dashes; a mark-paid records the payment against it), invoicing.bankDetails (a JSON string with any of accountName, accountNumber, bankName, referenceHint, all strings; accountNumber may only hold digits, dashes and spaces; ONE account, now only the fallback), and invoicing.bankDetailsByCurrency (a JSON string keyed by currency code, one account per currency the studio invoices in: NZD, GBP, USD, AUD, EUR and nothing else. Each account is an object with any of bankName, accountName, location, accountNumber, iban, sortCode, bsb, bankCode, branchCode, achRouting, fedwireRouting, swift, referenceHint, all strings, all trimmed; accountNumber may only hold digits, dashes and spaces, iban only letters, digits and spaces, and an account that names neither an accountNumber nor an iban is rejected because the How to pay block would then tell the client nowhere to send the money. A client invoice quotes the account matching the INVOICE\'s currency and falls back to invoicing.bankDetails when that currency has no entry), the five email delivery keys: email.deliveryMode (allowlist | all), email.allowedDomains (a JSON array of bare domains), email.allowedAddresses and email.blockedAddresses (JSON arrays of single mailboxes) and email.allowedOrgIds (a JSON array of organisation ids), and the three project-manager keys, each a team_members id or empty: studio.projectManagerId (set, and it overrides EVERY client, existing and new, as the lead shown everywhere; empty falls back to per-client assignments), studio.defaultProjectManagerId.retainer and studio.defaultProjectManagerId.project (the project manager assigned automatically when a NEW client is created, by whether it is a retainer or a one-off project; never touches an existing client and never overrides studio.projectManagerId while that is set). All three accept any non-blank id (an id that does not resolve to a real team member is treated as unset at resolve/assignment time, not rejected here) and reject only a whitespace-only value. An empty value clears any of the thirteen, and clearing email.deliveryMode turns the allowlist ON rather than off, because the default is closed. WIDENING ANY email.* KEY IS SUPER ADMIN ONLY and this token is not one: the route answers 403 with a sentence. Only closing the gate (email.deliveryMode = allowlist, or clearing it) is admin-writable, and every email.* write is recorded in the audit log with its old and new value.', {
    key: prop('string', 'Setting key'),
    value: prop('string', 'Setting value'),
  }, ['key', 'value']),
  tool('list_email_suppressions', 'List the last 100 recipients the email delivery allowlist held back, newest first. One row per withheld address: to, orgId, template, subject, reason and when. Reasons are not_in_allowlist (neither the address list nor the domain list covered them), address_blocked (on email.blockedAddresses, which is checked before every other rule) and not_an_address (the entry was not one parseable mailbox, e.g. two addresses typed into one box). Templates include the transports we do not own: clerk-org-invite is an invitation Clerk would have emailed itself, stripe-invoice is a bill Stripe would have emailed on finalise. This is how you answer "did that email actually reach them?": a withheld address produces a row here and no send at all. Read-only; clearing the log is super-admin only and is deliberately not exposed as a tool.'),
  ...
  tool('get_financial_snapshots', 'Read the stored monthly financial snapshots, oldest first: one row per month (monthKey YYYY-MM, UTC) with cashNzd, owedNzd, mrrNzd, activeClients, burnNzd, runwayMonths, source and capturedAt. source cron = the daily snapshot job\'s own reading: the current month\'s row is rewritten every day, and a past month\'s row holds its last write before that month closed. source backfill = rebuilt after the fact, where any field that could not be honestly rebuilt is null. A month with no row was never captured. The overview\'s MRR delta compares live MRR against the latest earlier month whose mrrNzd is not null.'),
  tool('snapshot_fill_month', 'Write ONE missing past month into financial_snapshots (source backfill), and nothing else: insert only, never an overwrite, no other month touched. Refused with nothing written: 409 when the month already has a row (whatever its source); 400 for the current month (the daily snapshot job owns it), a future month, a malformed month, or a month that ended before any Airwallex ledger, Xero P&L or invoice data we hold (code before_data); 422 when the Airwallex balances were read before the month ended (code balances_stale: run the Airwallex sync, then fill) or when neither month-end cash nor money owed can be rebuilt (code nothing_derivable; burn alone is recomputable from the P&L and is not written). Cash is rewound from the last synced Airwallex wallet balances through the ledger, and is null for any month that ended after Airwallex yield was first held, held today or not, because the Cash card counts yield and the yield held at a past month end is not stored. Which months those are comes from the setting finance.yieldFirstHeldAt: the Airwallex sync writes it as unknown the first time it sees a yield holding and never clears it, and unknown stops every month (so, until it exists, do any yield rows or a finance.yieldHoldings setting, even an emptied one); set it (update_settings) to the date of the first transfer into yield, YYYY-MM-DD read as midnight UTC (a day early is the safe side of New Zealand time; any earlier date is safe), and months that ended by then rebuild. Burn is the trailing three months of the stored Xero P&L, runway is cash over burn, and money owed is rebuilt from invoice issue and paid dates only when no invoice with money on it is left in doubt at month end by its dates and the invoice ledger reaches back to the month (otherwise null, never a made-up 0, with the blocking invoices and the certain-to-possible range listed). The dates can still mislead: an invoice first imported already issued counts from its source invoice date even if it was a draft in the source then. A month end is the first instant of the next month, UTC, and anything stamped exactly at it (a Xero date on the 1st) belongs to the next month. MRR and active clients are always null: no history of custom_mrr or organisation status is kept. The response gives each field\'s value and basis, or why it was left null. Logged as a snapshot-metrics run (see list_crons).', {
  ...
  switch (name) {
    ...
    case 'delete_client': {
      ...
      return json(await apiWrite(`/api/admin/clients/${clientId}`, token, 'DELETE', {
        ...
        dryRun: args.dryRun !== false,
        // Opt in only, written as === true so a string or a number still gets
        // the safe answer.
        removeClerkOrganisation: args.removeClerkOrganisation === true,
      }))
    }
    ...
    case 'backfill_invoice_numbers':
      return json(await apiWrite('/api/admin/invoices/backfill-numbers', token, 'POST', args as Record<string, unknown>))
    case 'update_invoice': {
