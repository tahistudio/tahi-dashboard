...
export const TOOLS: ToolDef[] = [
  ...
  tool('update_settings', 'Update a dashboard setting. Any key is accepted; ten are validated and answer 400 with a sentence when the value is wrong: invoicing.defaultChannel (stripe | xero), invoicing.xeroEmailMode (dashboard | xero | both, who emails a Xero-rail invoice), invoicing.xeroPaymentAccountCode (a Xero bank account code, up to 10 letters, digits or dashes; a mark-paid records the payment against it), invoicing.bankDetails (a JSON string with any of accountName, accountNumber, bankName, referenceHint, all strings; accountNumber may only hold digits, dashes and spaces; ONE account, now only the fallback), and invoicing.bankDetailsByCurrency (a JSON string keyed by currency code, one account per currency the studio invoices in: NZD, GBP, USD, AUD, EUR and nothing else. Each account is an object with any of bankName, accountName, location, accountNumber, iban, sortCode, bsb, bankCode, branchCode, achRouting, fedwireRouting, swift, referenceHint, all strings, all trimmed; accountNumber may only hold digits, dashes and spaces, iban only letters, digits and spaces, and an account that names neither an accountNumber nor an iban is rejected because the How to pay block would then tell the client nowhere to send the money. A client invoice quotes the account matching the INVOICE\'s currency and falls back to invoicing.bankDetails when that currency has no entry), and the five email delivery keys: email.deliveryMode (allowlist | all), email.allowedDomains (a JSON array of bare domains), email.allowedAddresses and email.blockedAddresses (JSON arrays of single mailboxes) and email.allowedOrgIds (a JSON array of organisation ids). An empty value clears any of them, and clearing email.deliveryMode turns the allowlist ON rather than off, because the default is closed. WIDENING ANY email.* KEY IS SUPER ADMIN ONLY and this token is not one: the route answers 403 with a sentence. Only closing the gate (email.deliveryMode = allowlist, or clearing it) is admin-writable, and every email.* write is recorded in the audit log with its old and new value.', {
