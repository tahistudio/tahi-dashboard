...
export const TOOLS: ToolDef[] = [
  ...
  tool('rebuild_task_suggestions', "Re-read call transcripts the suggester has already looked at, and add what the new read finds to what is already waiting. Nothing already filed is expired or changed: pending and snoozed suggestions stay in the inbox, and applied, rejected and failed ones are decisions and are left alone. The new read files only what is new; an item it proposes again, in the same words or different ones, is counted as a duplicate and the existing suggestion is kept, and nothing a founder already rejected comes back. The transcripts are read straight away (at most 20, oldest first for all; the rest wait for the scheduled sweep, reported as deferred), each read twice (the second read is shown the first and asked only for what it missed), and the sweep summary comes back as read. Returns mode, kept (undecided suggestions left in place), expired, transcripts, read and deferred. replace true is the escape hatch for a change to what the suggester knows how to propose: it expires the still-undecided suggestions first so the re-read starts from nothing. Costs roughly 10 cents per transcript read with both reads.", {
    ...
    read_now: prop('boolean', 'Read the transcripts straight away and return the summary. Default true; false only clears the read mark and leaves them for the next scheduled sweep.'),
  }),
