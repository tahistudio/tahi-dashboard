...
const TOOLS: ToolDef[] = [
  ...
  tool('update_client', 'Update a client organisation', {
    ...
    invoiceChannel: prop('string', 'How this client is billed: stripe (a Stripe invoice paid on the Stripe hosted page) or xero (a Xero invoice carrying its own pay-now link). Pass null to clear it and fall back to the studio default', { enum: ['stripe', 'xero'] }),
    paymentTerms: prop('string', 'When their invoices are due: card (card on file), net_7, net_14 or net_30. Pass null to leave it unset', { enum: ['card', 'net_7', 'net_14', 'net_30'] }),
  }, ['clientId']),
